...
::: aside
Unsupervised anomaly detection methods can identify attacks that significantly deviate from normal behavior, especially high-variance or clearly abnormal patterns. However, they struggle with low-signal attacks that resemble legitimate traffic, leading to missed detections. <br/> Their strength lies in detecting unknown or emerging threats where labeled data is unavailable, but their performance is inconsistent without careful threshold calibration. <br/>Overall, unsupervised methods are moderately effective but insufficient as a standalone intrusion detection strategy.<br/> High recall with low PR-AUC translates into excessive alerts, overwhelming analysts despite successful attack detection.
:::
...
::: aside
Supervised machine learning models significantly outperform statistical anomaly detection methods across all major evaluation metrics. They deliver higher accuracy, stronger precision and recall, and more consistent detection of diverse attack types. <br/> Statistical methods, such as Z-score, IQR, and Mahalanobis distance, catch only basic or extreme outliers and fail to capture complex attack behaviors. <br/>Supervised models also provide clearer interpretability through feature importance and decision boundary analysis, making them far easier for analysts to validate and operationalize. Overall, supervised learning offers far superior intrusion detection performance and reliability in real-world environments. <br/>  While supervised models achieved the strongest performance in this study, their effectiveness assumes high-quality labels and careful separation between training and test data. In real-world intrusion detection systems, labeling delays, noisy ground truth, or temporal leakage can reduce model reliability. This highlights the importance of combining supervised models with anomaly-based methods in operational deployments.
...
## Hybrid IDS Strategy (Proposed) {transition="fade-in slide-out" transition-speed="slow" .scrollable}
- **Stage 1: Unsupervised anomaly detection**
  - Autoencoder / Isolation Forest flag novel or low-frequency behavior
- **Stage 2: Supervised classification**
  - Random Forest assigns high-confidence attack labels
- **Stage 3: Explainability-driven validation**
  - SHAP supports analyst trust and feature-level inspection
- **Operational benefit**
  - Balances early detection, precision, and interpretability

::: aside
This is a practical hybrid intrusion detection strategy that synthesizes the strengths observed across models. <br/>
In the first stage, unsupervised detectors such as autoencoders or isolation forests act as lightweight anomaly tripwires. They are well suited for identifying previously unseen or low-frequency attacks without requiring labels. <br/>
In the second stage, supervised models—particularly Random Forest—are applied to traffic flagged as suspicious. This stage provides higher precision and stable classification for known attack patterns, reducing false positives. <br/>
In the final stage, SHAP explainability is used to support analyst validation by highlighting which features drove each detection decision. This helps distinguish true attacks from noise and increases operational trust. <br/>
Overall, this hybrid approach reflects real-world IDS constraints by combining adaptability, accuracy, and interpretability rather than relying on a single detection paradigm.
:::